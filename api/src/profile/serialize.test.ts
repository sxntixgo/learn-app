...
describe('the avatar: generated by default, uploaded when there is one (§11.1)', () => {
  ...
  it('becomes an upload, with a digest-busted URL, once the account has one', () => {
    const sha = 'a'.repeat(64);
    const payload = serializeProfile(modelFixture({ avatarSha: sha }), 'owner');

    expect(payload.avatar.kind).toBe('upload');
    expect(payload.avatar.url).toBe(`/api/v1/profiles/santiago/avatar?v=${sha.slice(0, 16)}`);
  });

  it('keeps the identicon seed alongside the upload, as the fallback face', () => {
    // A client whose image request fails — offline, a 404 after the owner
    // removed it mid-render — must have something to draw other than a hole
    // where a person's face goes.
    const payload = serializeProfile(modelFixture({ avatarSha: 'b'.repeat(64) }), 'owner');
    expect(payload.avatar.seed).toBe(avatarSeed(modelFixture().id));
  });

  it('publishes the upload to an anonymous reader too — it is profile-header data', () => {
    const sha = 'c'.repeat(64);
    const anonymous = serializeProfile(modelFixture({ avatarSha: sha }), 'anonymous');
    const owner = serializeProfile(modelFixture({ avatarSha: sha }), 'owner');
    expect(anonymous.avatar).toEqual(owner.avatar);
  });

  it('falls back to the identicon rather than publishing a URL built from nothing', () => {
    // The fail-safe direction, and not hypothetical: a fixture that simply
    // omitted `avatarSha` spread through `Partial<ProfileModel>` without
    // TypeScript flagging it, and produced `?v=undefined` in every payload.
    const broken = { ...modelFixture(), avatarSha: undefined } as unknown as Parameters<typeof serializeProfile>[0];
    const payload = serializeProfile(broken, 'anonymous');
    expect(payload.avatar.kind).toBe('identicon');
    expect(payload.avatar.url).toBeUndefined();
  });
});
